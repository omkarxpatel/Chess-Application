...
def create_board_surf():
    ...
    for y in range(8):
        for x in range(8):
            ...
            pygame.draw.rect(
                board_surf, pygame.Color("darkgrey" if dark else "beige"), rect
            )
            dark = not dark
        dark = not dark
    return board_surf
    ...
    for y in range(8):
        for x in range(8):
            ...
            if piece:
                ...
                color, type = piece
                s1 = font.render(
                    type[0], True, pygame.Color("red" if selected else color)
                )
                s2 = font.render(type[0], True, pygame.Color("darkgrey"))
                pos = pygame.Rect(
                    BOARD_POS[0] + x * TILESIZE + 1,
...
def draw_selector(screen, piece, x, y):
    if piece != None:
        rect = (
            BOARD_POS[0] + x * TILESIZE,
            BOARD_POS[1] + y * TILESIZE,
            TILESIZE,
            TILESIZE,
        )
...
def draw_drag(screen, board, selected_piece, font):
    if selected_piece:
        ...
        selected_rect = pygame.Rect(
            BOARD_POS[0] + selected_piece[1] * TILESIZE,
            BOARD_POS[1] + selected_piece[2] * TILESIZE,
            TILESIZE,
            TILESIZE,
        )
